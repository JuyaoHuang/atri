...
Token approximation (truncation only): ``max_tokens * 4`` chars in
:func:`_truncate`. ASCII-biased rough heuristic; under-counts CJK tokens
which pairs well with §8.7's conservative stance. ``tokens_freed`` is
reported as a raw character delta -- callers treat it as a loose byte-budget
indicator, never as an exact token count.
...
Token 近似估算（仅用于截断）：:func:`_truncate` 中按 ``max_tokens * 4``
字符计算。偏向 ASCII 的粗略启发式，对中文会低估 token 数，这与 §8.7
的保守立场相契合。``tokens_freed`` 以原始字符差报出——调用方将其作为
粗略的字节预算指示，而不是精确 token 计数。
"""
